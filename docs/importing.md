# Importing secrets into Doriath

Doriath can import an existing vault from other password managers. The whole
...
| **Bitwarden** | *Tools → Export vault* → `.json` or `.csv` | Login items are imported with their username, password, URL, notes, and TOTP seed; folders and collections become Doriath folders. Cards, identities, and secure notes are not imported (they appear in the rejected list). |
...
| **Doriath backup** | A `.doriath-backup` file | Restore an encrypted Doriath backup with its passphrase. |

### KDBX is not supported

Doriath does **not** read `.kdbx` files. KDBX is an encrypted binary container,
not an interchange format — supporting it would mean asking you to type your
KeePass master password into Doriath, which is exactly the phishing pattern we
...
2. **Mapping preview** — see the first rows and which source field maps to which
   Doriath field. Sensitive cells are masked; reveal them per cell. For generic
...
4. **Duplicates** — rows that match an existing secret (same name + URL) are
   ...
   gets an "(imported)" suffix) per row or in bulk. Doriath never overwrites an
   existing secret.
...
- **Shares, link shares, and delegations** — set these up again in Doriath.
- **Tags and attachments** — not part of the Doriath secret model.
