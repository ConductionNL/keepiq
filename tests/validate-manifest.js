...
// points at an existing page id — the ADR-024 / ADR-036 reachability gate).
//
// Usage:
...
//   3. node_modules/@conduction/nextcloud-vue/src/schemas/app-manifest.schema.json (v1 fallback)
//   4. ../nextcloud-vue/src/schemas/app-manifest-v2.schema.json (sibling worktree)
//   5. ../nextcloud-vue/src/schemas/app-manifest.schema.json (v1 fallback)
...
function structuralLint(manifest) {
	...
	const allowedTypes = new Set([
		'index', 'detail', 'dashboard', 'logs', 'settings',
		'chat', 'files', 'form', 'wiki', 'map', 'roadmap', 'search', 'custom',
	])
	...
	for (let i = 0; i < (manifest.pages || []).length; i++) {
		...
		if (page.type && !allowedTypes.has(page.type)) {
			errors.push(`pages[${i}].type: "${page.type}" not in v2 enum`)
		}
		if (page.id) {
 ...
 * matching page is a dead link. ADR-024 / ADR-036 reachability gate.
 *
