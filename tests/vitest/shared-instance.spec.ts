/*
 * SPDX-FileCopyrightText: 2026 Keepiq Contributors
...
describe(`${APP_ID} shared-instance guard`, () => {
	beforeEach(() => {
		// The guard exempts CI, because on a runner localhost:8080 is the
		// runner's own throwaway Nextcloud. These cases are about what happens
		// OFF CI, and vitest runs with CI and GITHUB_ACTIONS set on a runner, so
		// without this the three refusal cases pass on a laptop and fail on every
		// runner. Emptying both makes the test read the same in both places.
		vi.stubEnv('CI', '')
