...
test.describe('Lock screen — spec: encryption-suites/spec.md', () => {
	test('renders unlock form with password field and Unlock button', async ({ page }) => {
		...
		await expect(
			page.locator('.lock-screen button').filter({ hasText: /Unlock|Set up vault/i }).first(),
		).toBeVisible()
	...
	test('strength meter appears and validates while typing on setup mode only', async ({ page }) => {
		...
		if (/Set up your master password/i.test(heading)) {
			...
			await page.locator('.lock-screen input[type="password"]').first().fill('password')
			await expect(page.locator('.password-strength-meter')).toBeVisible({ timeout: 5_000 })
			expect(await page.locator('.lock-screen input[type="password"]').count())
				.toBeGreaterThanOrEqual(2)
			await expect(
				page.locator('.lock-screen button').filter({ hasText: /Set up vault/i }),
			).toBeDisabled()
		} else {
			// Unlock mode: exactly one password field, button enables once filled.
			const btn = page.locator('.lock-screen button').filter({ hasText: /^\s*Unlock\s*$/i }).first()
			await expect(btn).toBeDisabled()
			await page.locator('.lock-screen input[type="password"]').first().fill('anything')
			await expect(btn).toBeEnabled()
		}
	})

	test('wrong master password shows an error and stays on the lock screen', async ({ page }) => {
		await gotoLock(page)
		await expect(lockHeading(page)).toBeVisible({ timeout: 15_000 })
		const heading = (await lockHeading(page).textContent())?.trim() ?? ''
