...
function mockApi({ owned = [], missing = [] } = {}) {
	vi.spyOn(axios, 'get').mockImplementation((url) => {
		if (url.includes('/reconcile')) {
			return Promise.resolve({ data: { secrets: [], recipients: [], missing } })
		}
...
describe('TeamFolderDialog', () => {
	...
	it('offers to share an unshared folder', async () => {
		...
		expect(wrapper.find('[data-testid="team-folder-members"]').exists()).toBe(false)
	})
	...
	it('renders the member list and add-member controls for a shared folder', async () => {
		...
		wrapper.vm.refresh()
		await flush()
		expect(wrapper.find('[data-testid="team-folder-members"]').exists()).toBe(true)
		expect(wrapper.findAll('.team-folder-dialog__member')).toHaveLength(2)
		expect(wrapper.find('[data-testid="team-folder-add-member"]').exists()).toBe(true)
		expect(wrapper.find('[data-testid="team-folder-unshare"]').exists()).toBe(true)
	})

	it('shows the needs-reshare warning when the reconcile pass reports missing pairs', async () => {
		mockApi({
			owned: [{ id: 'tf-1', folderId: 'folder-1', folderName: 'DevOps', members: [] }],
			missing: [
		...
		expect(wrapper.find('[data-testid="team-folder-run-fanout"]').exists()).toBe(true)
	})
})
