...
class CertificateAuthorityService
{
    ...
    public function bootstrap(): void
    {
        ...
        if ($root !== null) {
            try {
                ...
                // Rows existing is NOT the same as the CA working. The stored
                // private keys are sealed with ICrypto, which is keyed on the
                // instance `secret` in config.php. If that secret changed —
                // config.php regenerated by a container entrypoint, restored
                // from a different backup than the database, re-templated by
                // config management — every CA key becomes permanently
                // undecryptable while the rows still look perfectly intact.
                //
                // Reporting `healthy` here is what previously let suite
                // creation sail through its health gate and then die deep in
                // signPublicKey() with a raw "HMAC does not match.".
                if ($this->isKeyMaterialUsable(certificate: $intermediate) === false) {
                    $this->recoverUnreadableCa();
                    return;
                }
        ...
        $this->performFreshBootstrap();
    }//end bootstrap()

    /**
     * Generate a brand-new root + intermediate and mark the CA healthy.
     *
     * Split out of bootstrap() so the unreadable-key recovery path can reach it
     * WITHOUT re-entering bootstrap(). Recursing there would only terminate
     * because findRoot() starts throwing once the rows are deleted — i.e. the
     * loop's exit condition would live in the database rather than in the code.
     *
     * @return void
     *
     * @SuppressWarnings(PHPMD.UndefinedVariable) PHP's openssl_x509_export / openssl_pkey_export
     ...
     * @spec openspec/changes/retrofit-2026-05-25-doriath-coverage/tasks.md#task-1
     */
    private function performFreshBootstrap(): void
    {
    ...
    }//end performFreshBootstrap()

    /**
     * Whether a stored CA private key can still be decrypted.
     *
     * ICrypto seals CA private keys with the instance `secret`. Nextcloud
     * never rotates that secret itself, but it is lost whenever config.php is
     * regenerated or restored out of step with the database — a routine
     * container/backup failure mode. The row survives; the key does not.
     *
     * A missing key is treated as unusable too: an intermediate with no
     * private key cannot sign, which is the same operational outcome.
     *
     * Never throws — callers use this to DECIDE how to report health, so it
     * must not itself become the failure it is detecting.
     *
     * @param CACertificate $certificate The CA certificate row to probe.
     *
     * @return bool True when the private key decrypts.
     *
     * @spec openspec/specs/encryption-suites/spec.md#requirement-ca-hierarchy
     */
    private function isKeyMaterialUsable(CACertificate $certificate): bool
    {
        $sealed = $certificate->getPrivateKey();
        if ($sealed === null || $sealed === '') {
            return false;
        }

        try {
            $pem = $this->crypto->decrypt($sealed);
        } catch (Throwable $e) {
            $this->logger->error(
                'Doriath: CA private key cannot be decrypted — the instance secret in config.php '
                .'has almost certainly changed since the key was written',
                ['exception' => $e, 'certificateId' => $certificate->getId()]
            );
            return false;
        }

        // Decryption succeeding is necessary but not sufficient — the
        // plaintext must still parse as a private key before we call the CA
        // healthy.
        return openssl_pkey_get_private(private_key: $pem) !== false;
    }//end isKeyMaterialUsable()

    /**
     * Handle a CA whose stored private key can no longer be decrypted.
     *
     * Regenerating is only safe when nothing has been issued yet. Every
     * existing suite holds a certificate signed by the CURRENT intermediate,
     * so minting a fresh root would orphan that chain — the suites keep
     * working for decryption (user secrets are sealed with the user's own RSA
     * key, wrapped by the master password, never by the instance secret) but
     * their certificates would no longer verify against the new root.
     *
     * So: an empty CA regenerates itself silently, and a populated one is
     * marked degraded and left alone for an operator to migrate deliberately.
     * Choosing for them here would be choosing to break their chain.
     *
     * @return void
     *
     * @spec openspec/specs/encryption-suites/spec.md#requirement-ca-hierarchy
     */
    private function recoverUnreadableCa(): void
    {
        $userSuites  = $this->suiteMapper->countActiveByOwnerType('user');
        $appSuites   = $this->suiteMapper->countActiveByOwnerType('application');
        $issuedCount = ($userSuites + $appSuites);

        if ($issuedCount > 0) {
            $this->setDegraded();
            $this->logger->error(
                'Doriath: CA private key is unreadable and {count} active suite(s) are chained to it. '
                .'Refusing to regenerate automatically — a new root would orphan every issued certificate. '
                .'Restore the original config.php `secret`, or re-issue the affected suites deliberately.',
                ['count' => $issuedCount]
            );
            return;
        }
        ...
        $this->logger->warning(
            'Doriath: CA private key is unreadable and no suites have been issued — regenerating the CA'
        );

        // Nothing is chained to the old material, so clear it out and build a
        // fresh CA directly — not via bootstrap(), which would re-read the rows
        // we just deleted.
        $this->caCertificateMapper->deleteAll();
        $this->performFreshBootstrap();
    }//end recoverUnreadableCa()
    ...
    public function getStatus(): array
    {
        ...
        // An intermediate whose private key no longer decrypts cannot sign
        // anything, so it is not "healthy" no matter how far off its expiry
        // is. Reported ahead of the expiry checks below because it outranks
        // them: an unreadable key is already broken, not soon-to-be.
        if ($this->isKeyMaterialUsable(certificate: $intermediate) === false) {
            return [
