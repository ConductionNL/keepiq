 ...
 * `keepiq`. Every schema this app already owns still carries
 * `application = 'doriath'`, so the pair matches nothing — and
 ...
 * `decidesk`, 21 under `softwarecatalog`, 17 under `openbuild`. `doriath` did
 * NOT appear in that measurement — that instance carries no schemas under the
 * old id, which this step reports as "nothing to do" and writes nothing for.
 * The step ships anyway because the instance that HAS such rows is a different
 * one, and the failure mode there is silent: no error, just empty collections.
 *
 ...
 *
 * @spec exclude No canonical spec covers the `doriath` -> `keepiq` schema
...
class MigrateSchemaApplicationId implements IRepairStep {
	 ...
	 * @var string
	 */
	public const OLD_APP_ID = 'doriath';
	 ...
	 * @var string
	 */
	public const NEW_APP_ID = 'keepiq';
	...
	public function getName(): string {
		return 'Move Keepiq schemas onto the keepiq application id';
	}//end getName()
