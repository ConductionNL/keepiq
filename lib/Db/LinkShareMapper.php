...
class LinkShareMapper extends QBMapper
{
    ...
    /**
     * Find a link share by its primary key.
     *
    ...
    /**
     * Atomically increment usage_count only when below the usage limit.
     *
     * Implements the race-safe `UPDATE ... WHERE usage_count < usage_limit`
     * from design.md D3. Returns the number of affected rows: 1 when the
     * increment succeeded, 0 when the limit was already reached.
     *
     * @param string $id The link share ID
     *
     * @return int The number of affected rows
     */
    public function incrementUsageCountIfBelowLimit(string $id): int
    {
        $qb = $this->db->getQueryBuilder();
        $qb->update($this->getTableName())
            ...
            ->set('blob_fetched', $qb->createNamedParameter(false, \OCP\DB\QueryBuilder\IQueryBuilder::PARAM_BOOL))
            ->where($qb->expr()->eq('id', $qb->createNamedParameter($id)))
            ->andWhere($qb->expr()->lt('usage_count', 'usage_limit'));

        return $qb->executeStatement();
    }//end incrementUsageCountIfBelowLimit()
}//end class
