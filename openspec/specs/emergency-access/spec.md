...
@e2e exclude State-machine/authorization contract — covered by PHPUnit EmergencyAccessServiceTest (designate/request/decline/approve-by-timeout + the approved+grantee release gate with identical wrong-state/wrong-caller refusal). CORRECTION 2026-08-11: the previous sentence here deferred a live Playwright run because "the worktree is not deployed and deploying to the shared dev instance is prohibited". That reason was never valid and is now demonstrably dead — the shared dev instance was never the target: the E2E Tests (Playwright) job provisions its OWN throwaway Nextcloud (nextcloud-test-refs stable32, seeded by tests/e2e/ci-seed.sh) and already passes in this repo. src/views/EmergencyAccessView.vue is routed at /emergency-access with an "Emergency access" menu entry in src/manifest.json, is navigable, and already carries data-testid hooks (emergency-access-view, emergency-access-designate, emergency-grantee-input, emergency-wait-select, emergency-master-input). The DOM flow is therefore GENUINELY UNCOVERED, not excluded — this waiver stands only for the server-side state machine above, which is not DOM-observable. Writing that Playwright spec is open work; nothing here should be read as claiming it exists.
...
@e2e exclude Server-side suite rotation/revocation listener contract — covered by PHPUnit (invalidateForGrantorRotation/clearForGrantorRevocation/invalidateForGranteeRevocation + invalidated audit). Those three listener methods are each exercised in tests/Unit/Service/EmergencyAccessServiceTest.php (testRotationInvalidatesGrantorEnvelopes, testRevocationClearsGrantorEnvelopes, testGranteeRevocationInvalidatesEnvelopes), which runs in the "Unit Tests" suite of phpunit.xml. CORRECTION 2026-08-11: the previous trailing clause, "Live UI run deferred (worktree not deployed)", was never a valid reason — the E2E Tests (Playwright) job provisions its own throwaway Nextcloud and already passes here, so no deployment of a worktree to any shared instance was ever required. A listener firing on a rotation/revocation event has no DOM surface of its own, which is why this exclusion stands; the dead excuse is removed rather than carried forward.
