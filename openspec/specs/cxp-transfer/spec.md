...
- GIVEN an HPKE-sealed CXF envelope addressed to Keepiq's ephemeral public key
- WHEN Keepiq receives it
- THEN the system MUST open it in the browser and transmit no plaintext or opening private key to the server or relay

### Requirement: Keepiq as importing provider
The system MUST let Keepiq generate a keypair, produce a CXP request, receive and client-side-decrypt the sealed CXF payload, and feed it into the existing CXF import pipeline, writing no plaintext file to disk.

#### Scenario: Sealed CXF imports without a plaintext file
- GIVEN a cooperating exporting provider and a Keepiq CXP import request
- WHEN Keepiq opens the sealed CXF payload
- THEN the decrypted CXF MUST flow through the existing import pipeline and no plaintext file MUST be written to disk

### Requirement: Keepiq as exporting provider
The system MUST let Keepiq receive a CXP request, gate the export with the existing fresh master-password re-auth, assemble the CXF payload client-side via the existing export path, HPKE-seal it under the requester's public key, and return only the sealed envelope, writing no plaintext file to disk.

#### Scenario: Only a sealed envelope leaves Keepiq
- GIVEN a re-authenticated CXP export
- WHEN Keepiq assembles the CXF payload
...
- As a user leaving Keepiq, I want to hand my vault directly to the destination provider sealed so that I never create a plaintext export file to forget about.
