...
- **Expiry still is not swept** (`secret-request-expiry-lifecycle` is unbuilt), so lapsed requests will appear as pending here too. The list should judge expiry on `expires_at`, as the user-side copy action already does.
