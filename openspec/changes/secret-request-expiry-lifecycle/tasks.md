...
- [x] 1.1 Pre-fill the expiry field in `SecretRequestCreateDialog.vue` with a suggested value the requester can change or clear.
...
- [x] 2.1 Add the terminal `STATUS_EXPIRED` to `SecretRequest`.
  - `status` is already a string column, so no migration is required
- [x] 2.2 Add the expire transition to `SecretRequestService`: invalidate the token, delete the placeholder for a fresh request, preserve the Secret and its values for a re-request, and dispatch the audit event with the SYSTEM as actor rather than the requester.
- [x] 2.3 Add a `TimedJob` that sweeps lapsed requests and register it in `appinfo/info.xml`, following `ExpireMachineLeasesJob` (hourly).
...
- [x] 3.1 Hoist the expiry evaluation in `SecretRequestPolicy::requireOpenByToken()` above the status switch so it applies to any status.
  - A request that lapsed since the last sweep is refused on `expires_at` alone — the job is cleanup, never enforcement
- [x] 3.2 Give `expired` an explicit arm in that switch, in the 410 family alongside `fulfilled` and `declined`.
...
- [x] 3.3 Render the expired state in `SecretRequestFill.vue` as an expiry message rather than an unexpected error.

## 4. Tests

- [x] 4.1 PHPUnit for the expire transition: placeholder deleted for a fresh request, Secret and values preserved for a re-request, `expired` is terminal, and the audit actor is the system.
- [x] 4.2 PHPUnit for the job: sweeps only lapsed pending requests, never touches a request with no `expires_at`, and is idempotent across consecutive runs.
- [x] 4.3 PHPUnit for the access gate: a lapsed-but-unswept request is refused on `expires_at` alone, an `expired` request reports expiry rather than a 500, and every other status keeps the code it returns today.
- [x] 4.4 Vitest for the dialog and fill view: expiry pre-filled, clearable to null, and the expired state renders as an expiry message.
- [x] 4.5 Verify each new/modified spec scenario is driven by a test or carries a reason-bearing `@e2e exclude`, so gate-19 measures this change.
