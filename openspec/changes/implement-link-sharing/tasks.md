...
- [x] 8.1 DONE W31. `src/views/LinkShareAccess.vue` ships the public recipient-facing access page wired into the manifest-v2 declarative shell (page id `LinkShareAccess`, route `/share/link/:token`, type `custom`). Component drives the two-phase protocol: `mounted()` reads `:token` from `$route.params` (with a URL-path fallback for non-router mounts), calls `useLinkShareStore.fetchPublicLinkShare(token)` for Phase 1, renders a password form, and on submit calls `decryptPublicSnapshot(share, password)` (Argon2id-AES-GCM) + `confirmPublicLinkShare(token)` for Phase 2. A failed decrypt records `priorFailure=true` and re-fetches with `failed=true` so the brute-force counter ticks server-side. A 404 from the controller surfaces as a uniform "not available" terminal state. Component + page entry registered in `src/registry.js` (`kind:"page"`).
...
- [x] 9.1 DONE W31. The public `/share/link/:token` route lands as the `LinkShareAccess` page entry in `src/manifest.json` (`type:"custom"`, `component:"LinkShareAccess"`, `route:"/share/link/:token"`) — the manifest-v2 shell (`CnAppRoot` + `CnPageRenderer`) generates the route from the manifest, so there is no per-app `src/router/index.js` to edit. The token is exposed via `$route.params.token`. Matches the `SecretRequestFill` pattern shipped by `implement-secret-requests` §8.1.
- [x] 9.2 DONE W31. `PUBLIC_ROUTE_NAMES` in `src/App.vue` now includes `'LinkShareAccess'` alongside `'SecretRequestFill'`. `isPublicRoute()` short-circuits the lock-screen redirect watcher for any route in the allow-list, so an external recipient with no NC session can complete the password-protected access flow without being redirected to the master-password unlock screen — mirroring the `SecretRequestFill` behaviour.
...
- [x] 13.3 DONE W31. `tests/views/LinkShareAccess.spec.js` (4 tests) mounts the new view under jsdom: (a) Phase 1 — `mounted()` parses the token from the URL path and calls `useLinkShareStore.fetchPublicLinkShare('tok-abc', false)`, surfaces `share.encryptedSecretSnapshot` and renders the password form; (b) Phase 2 — submitting the password delegates to `decryptPublicSnapshot()` then POSTs `confirmPublicLinkShare()` and renders the snapshot view (form replaced); (c) wrong-password path surfaces the "password does not match" error, does NOT POST confirm, and re-fetches with `failed=true` so the server-side brute-force counter ticks; (d) 404 on Phase 1 fetch renders the uniform "not available" terminal state and suppresses the form.
