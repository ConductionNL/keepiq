...
- [~] 1.2 [DEFERRED — depends on `SeedDevelopmentSecrets` from the unbuilt implement-secrets] Create `SeedDevelopmentLinkShares` IRepairStep (debug-only) seeding example link shares for dev secrets — deferred to downstream cycle (handoff)
- [~] 1.3 [DEFERRED — depends on 1.2] Register `SeedDevelopmentLinkShares` as a post-migration repair step in `info.xml` — deferred to downstream cycle (handoff)
...
- [~] 6.1 [DEFERRED — needs install + build verification, only useful with the secrets feature] Add `argon2-browser` npm dependency to `package.json` — deferred to downstream cycle (handoff)
- [~] 6.2 [DEFERRED — webpack already uses `asset/resource`; the `.wasm` rule lands with 6.1] Configure webpack to handle WASM files — deferred to downstream cycle (handoff)
...
- [~] 8.1 [DEFERRED — standalone public page is mounted by the secrets SPA router which does not exist yet] `src/views/LinkShareAccess.vue` — deferred to downstream cycle (handoff)
- [~] 8.2 [DEFERRED — opened from the not-yet-existing SecretDetail view] `src/components/LinkShareCreateDialog.vue` — deferred to downstream cycle (handoff)
- [~] 8.3 [DEFERRED — rendered inside SecretDetail] `src/components/LinkShareList.vue` — deferred to downstream cycle (handoff)
- [~] 8.4 [DEFERRED — rendered by the create dialog] `src/components/LinkSharePasswordDisplay.vue` — deferred to downstream cycle (handoff)

## 9. Vue Router Integration

- [~] 9.1 [DEFERRED — this app uses the manifest-v2 declarative shell; there is no `src/router/index.js`. The public `/share/link/:token` page integrates with the secrets SPA when it lands] Add the public route — deferred to downstream cycle (handoff)
- [~] 9.2 [DEFERRED — depends on 9.1] Update the lock-screen guard to skip `meta.public` routes — deferred to downstream cycle (handoff)
- [~] 9.3 [DEFERRED — SecretDetail view does not exist] Integrate LinkShareCreateDialog into SecretDetail — deferred to downstream cycle (handoff)
- [~] 9.4 [DEFERRED — SecretDetail view does not exist] Integrate LinkShareList into SecretDetail — deferred to downstream cycle (handoff)
...
- [~] 12.7 [DEFERRED — secret deletion path is in the unbuilt SecretService] End-to-end secret-deletion cascade test — deferred to downstream cycle (handoff)
- [x] 12.8 Public payload tests assert only the encrypted blob/salt are returned, never decrypted data or owner identity

## 13. Frontend Tests

- [~] 13.1 [DEFERRED — this app has no JS test runner (no vitest/jest); adding one is out of scope for this change] argon2.js round-trip tests — deferred to downstream cycle (handoff)
- [~] 13.2 [DEFERRED — no JS test runner] useLinkShareStore tests — deferred to downstream cycle (handoff)
- [~] 13.3 [DEFERRED — no JS test runner + component depends on the secrets SPA] LinkShareAccess component tests — deferred to downstream cycle (handoff)
- [~] 13.4 [DEFERRED — no JS test runner + depends on SecretDetail] LinkShareCreateDialog component tests — deferred to downstream cycle (handoff)
- [~] 13.5 [DEFERRED — no JS test runner + depends on SecretDetail] LinkShareList component tests — deferred to downstream cycle (handoff)
