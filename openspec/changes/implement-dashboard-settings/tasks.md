...
- [~] 4.4 Update `src/views/settings/Settings.vue` — replace register form with three sections: PasswordPolicySection, CaHealthSection, ApplicationQueueSection; remove register-related form logic; use settings store to fetch and save admin settings — DEFERRED: this app's admin entry is now `src/views/AdminApplicationsView.vue` + `src/views/settings/AdminRoot.vue`; a wholesale `Settings.vue` rewrite is out-of-scope for W23 and lives with the next admin-IA pass.
- [~] 4.5 Update `src/views/settings/AdminRoot.vue` — retain CnVersionInfoCard header; ensure storesReady gates the updated Settings component; no other structural changes needed — DEFERRED: covered by the same admin-IA follow-up; current AdminRoot already gates on storesReady (W15-A) and renders the admin applications surface.
