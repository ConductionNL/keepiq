...
- [ ] 6.4 PHPUnit (flag): `possibly_compromised_at` is raised on every migrated row, idempotent on retry, never raised for failed rows, preserved through metadata edits, cleared on value replacement and propagated to shares; `SuiteCompromiseListener` de-duplicates notifications across a large migration
  - The **raise** half is done (`MigrationWorkServiceTest`: raised on commit, idempotent on retry, never for a failed row) and so is the notification de-duplication (`SuiteCompromiseListenerTest`: one notification per owner across a 500-secret migration, unflagged rows stay silent). The **preserve/clear** half — metadata edits leave the flag set, a value replacement clears it and propagates to shares — tests behaviour that task 4.1 has yet to implement, so it lands with 4.1 rather than being written against absent code.
