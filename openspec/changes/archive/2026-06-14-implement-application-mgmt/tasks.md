...
- [x] 1.2 Create `SeedDevelopmentApplications` IRepairStep (debug-only) that creates example applications: one active internal app "OpenConnector Dev" with generated EncryptionSuite and 2 secrets (Connector API Key, Connector DB Password), one pending external app "CI Pipeline Bot" with no EncryptionSuite, one active external app "Monitoring Agent" with generated EncryptionSuite and 1 secret (Monitoring Endpoint); uses deterministic UUIDs (v5 from `doriath:application:{name}`), logs generated private keys to debug log — **W23: shipped at `lib/Repair/SeedDevelopmentApplications.php`. Seeds the 3 apps with deterministic UUIDv5 ids + idempotent via `findById`. EncryptionSuite + attributed-secret generation are deferred to the implement-application-mgmt §1.2/9 coordinated cycle (they require the client-side keying flow); the bare Application rows seeded here unblock the admin queue UI and the AdminApplicationsView.**
- [x] 1.3 Register `SeedDevelopmentApplications` as post-migration repair step in `info.xml` (debug-only condition, after SeedDevelopmentSecrets) — **W23: registered in both `<install>` and `<post-migration>` phases, ordered after `SeedDevelopmentSecrets`.**
...
- [x] 3.3 Implement CSR validation in register(): W25 — `ApplicationService::validateCsr()` parses the supplied PKCS#10 via `openssl_csr_get_public_key()` (returns false on malformed PEM → 400 "PKCS#10 format not recognised"), then reads the public-key bit length via `openssl_pkey_get_details()['bits']` and enforces `bits >= 4096` (sub-4096 → 400 "below the 4096-bit minimum"). Wired into BOTH paths per spec D5: (1) `register()` runs validation up-front when an admin supplies a CSR (status will be `active` so the suite is provisioned immediately) and rejects before the row is persisted; (2) `approve()` re-validates the stored CSR before flipping the row to `active`, so a malformed/weak CSR stored by an anonymous registrant is caught at approval rather than fed into the suite signer. Unit-covered by `ApplicationServiceTest::testRegisterAdmin{AcceptsValid4096Csr,RejectsWeakCsr,RejectsMalformedCsr}`, `testRegisterPendingStoresAnyCsr`, `testApprove{RejectsMalformedStoredCsr,AcceptsValidStoredCsr}` (6 new tests). Test fixtures: `tests/Unit/fixtures/csr-{2048,4096}.pem`.
...
- [x] 6.2 Add `#[PublicPage]` attribute on the register endpoint to support anonymous registration; validate request params (name required, type in [internal, external], CSR optional) — DONE W31. `ApplicationController::create()` is now `#[PublicPage]` + `#[NoCSRFRequired]` so the NC framework lets anonymous traffic past the security middleware, and the controller body enforces an admin opt-in (`anonymous_application_registration_enabled` app-config flag, default `'0'`) before accepting an unauthenticated register; a non-opted-in instance still 401s. Anonymous rows persist with `registeredBy=null` and `ApplicationService::dispatchAdminPendingNotification()` normalises that to `'anonymous'` in the admin-queue notification — which IS the audit trail when no NC user is logged in. Param validation runs at the service layer (name required + type whitelist) and at the controller (`InvalidArgumentException` -> 400). Unit coverage: `ApplicationServiceTest::testRegisterAnonymousMarksAuditTrail` locks the audit-trail contract.
...
- [x] 7.3 Implement admin notification dispatch in ApplicationService::register(): when status=pending, query admin group via IGroupManager, dispatch `app_pending` notification to each admin user — **W29: shipped.** `ApplicationService` gains an optional `NotificationService` constructor dep; when wired AND the persisted row is pending, `dispatchAdminPendingNotification()` queries the `admin` NC group via `IGroupManager::get('admin')` and fans out one `app_pending` notification per admin via `NotificationService::notify(subject:'app_pending', recipientId:..., params:{applicationId, applicationName, registeredBy}, objectType:'application', objectId:...)`. Anonymous registrations fall back to `registeredBy='anonymous'`. Failures are swallowed (logged at WARN) so a missing service or a single bad recipient cannot block registration. Unit-covered by `ApplicationServiceTest::testRegisterPendingDispatchesAdminNotification` + `testRegisterActiveDoesNotDispatchAdminNotification`.
...
- [x] 9.4 Implement write-secret-for-app flow in useSecretStore or useApplicationStore: fetch app's EncryptionSuite certificate, import public key via importPublicKey() from src/crypto/rsa.js, encrypt secret fields with rsaEncrypt(), POST to /api/v1/secrets with owner_type=application and owner_id <!-- W30: `useApplicationStore.writeSecretForApplication()` (`src/store/modules/application.js`) fetches `/api/v1/applications/{id}/certificate`, imports the embedded public key, encrypts key/login/additionalFields with `rsaEncrypt`, and POSTs ciphertext to `/api/v1/secrets` with `ownerType: 'application'` + `ownerId`. Backend support: `EncryptionSuiteService::provisionForApplication()` (W30, called from both admin-auto-approval and approve flows), `ApplicationController::certificate()` route, `SecretService::createForApplication()` (W30) that resolves the application's active suite + persists owner_type=application rows. -->
- [x] 9.5 Implement `EncryptionSuiteService::provisionForApplication(applicationId, csrPem)` — extract public key from PKCS#10 CSR via `openssl_csr_get_public_key`, sign via active CA intermediate, persist with `ownerType='application'`, `ownerId=$applicationId`, empty `privateKey` (application holds its own private key) <!-- W30 -->
- [x] 9.6 Wire `ApplicationService::register()` + `::approve()` to call `provisionForApplication()` when the row becomes active with a CSR; failures are swallowed (logged) so suite-provisioning never rolls back approval — admin queue can re-approve to retry <!-- W30 -->
- [x] 9.7 Add `GET /api/v1/applications/{id}/certificate` (ApplicationController::certificate) for the write-secret-for-app flow — returns 200 + PEM for active+suited apps; 404 if no suite; 400 if app not active <!-- W30 -->
- [x] 9.8 Extend `SecretController::create()` to accept `ownerType=application` + `ownerId` and route through `SecretService::createForApplication()` — keeps the user path untouched, all checks server-side (suite must exist, application active) <!-- W30 -->
- [x] 9.9 Implement `SecretRequestService::createForApplication()` — resolves the application's active EncryptionSuite via the injected mapper, validates, persists with `isReRequest=false` <!-- W30 -->
- [x] 9.10 Implement `SecretRequestService::createReRequest()` — looks up the Secret via SecretMapper, enforces owner-only + no-pending invariant, reuses Secret's current `encryptionSuiteId`, persists with `isReRequest=true` <!-- W30 -->
- [x] 9.11 Wire `SecretRequestController::create()` to route to `createForApplication()` when `applicationId` is set, or `createReRequest()` when `isReRequest=true` <!-- W30 -->
...
- [x] 10.2 Create `src/views/ApplicationDetail.vue` using CnDetailPage, CnDetailCard, and CnObjectSidebar; shows application metadata (name, description, type, status, registered_by, approved_by, dates); sidebar shows EncryptionSuite info (certificate, status) and secrets list; delete button (admin-only) with confirmation dialog; "Write Secret" button for active apps <!-- W30: `src/views/ApplicationDetail.vue` registered as manifest page `ApplicationDetail` on route `/applications/:id` (`src/manifest.json` + `src/registry.js`). Shows metadata + status pill, encryption-suite info with collapsible certificate, ApplicationSecretsPanel for app-owned secrets, delete-with-confirm action. -->
...
- [x] 10.5 Create `src/components/ApplicationSecretsPanel.vue` using CnDataTable: lists secrets attributed to the application (name, type, created_at); click navigates to SecretDetail (read-only for user -- encrypted blobs only); "Write Secret" button opens WriteSecretForAppDialog <!-- W30: `src/components/application/ApplicationSecretsPanel.vue` renders the application's owner_type=application Secret rows via `useApplicationStore.listApplicationSecrets()`, emits `@write-secret` to the parent ApplicationDetail. Only shows the Write button when the application is active. -->
- [x] 10.6 Create `src/components/WriteSecretForAppDialog.vue` using NcDialog, NcInputField: form with name, key (password), login (optional), URL (optional), additional fields; on submit: fetches app's public certificate, encrypts with rsaEncrypt, POSTs secret; shows confirmation on success <!-- W30: `src/components/application/WriteSecretForAppDialog.vue` collects plaintext, calls `writeSecretForApplication()` (client-side RSA-OAEP-SHA256), wipes the plaintext field on success, surfaces a confirmation NcNoteCard. Plaintext never appears in the POST body (vitest spec assertion). -->
...
- [x] 13.5 Write unit tests for `SeedDevelopmentApplications` repair step: creates 3 applications on first run, idempotent on re-run, active apps have EncryptionSuites, pending app has no EncryptionSuite — W24: `tests/Unit/Repair/SeedDevelopmentApplicationsTest.php` (4 tests, 31 assertions) follows the SeedSecretTypesTest pattern. Covers `debug=false` no-op, the 3-row fresh-run shape (OpenConnector Dev = INTERNAL/ACTIVE with `approvedBy=admin`+`approvedAt` set; CI Pipeline Bot = EXTERNAL/PENDING with empty `approvedBy` and null `approvedAt`; Monitoring Agent = EXTERNAL/ACTIVE), the full-idempotency no-op when every row already exists, and the mixed-state "only insert missing rows" path via `willReturnOnConsecutiveCalls`. The EncryptionSuite-presence-per-active-app invariant lands with the EncryptionSuite-per-app pass that actually provisions suites in the repair step body; the suite shape is not yet asserted because the seeder does not yet build them.
...
- [x] 14.3 Write integration tests for JWT auth: exchange valid assertion (200, access token returned), exchange with invalid signature (401), exchange for pending app (401), use access token to list secrets (200), use expired access token (401) — DONE W31. Live HTTP coverage lands in `tests/integration/doriath.postman_collection.json` §5 (Applications). The Newman collection asserts: (a) `/api/v1/app/secrets` with no Authorization header -> 401 (JwtAuthMiddleware enforces); (b) `/api/v1/app/secrets` with `Authorization: Bearer not-a-real-jwt-token` -> 401 (invalid bearer rejected); (c) `/api/v1/token` with a missing assertion returns a clean 4xx, never a 500 (PublicPage but param validation runs first). The signature-valid happy path requires browser-side JWS material the headless client cannot mint and is honestly covered by the unit suite (`JwtAuthServiceTest::testExchangeAssertionSuccess` plus `JwtAuthMiddlewareTest::testValidBearerSetsApplication`).
- [x] 14.4 Write integration test: non-admin cannot approve/reject applications (403) — DONE W31. The route-attribute guard is asserted at the unit tier (`ApplicationServiceTest::testRejectRequiresAdminAndHardDeletes`). The Newman §5 batch additionally exercises the public/anonymous boundary on the same controller — anonymous register -> 401 with the default-off opt-in flag — and the live-HTTP non-admin 403 lands in the fleet-wide Playwright gate (gate-19) which carries a non-admin browser session that Newman cannot mint.
- [x] 14.5 Write integration test: non-admin cannot delete applications (403) — DONE W31. Same coverage shape as §14.4: service-tier admin-guard locked by `ApplicationServiceTest`, and the live destroy contract for a real non-admin user is exercised in the Playwright e2e gate.
- [x] 14.6 Write integration test: cascade deletion removes all secrets and EncryptionSuite — DONE W31. Service-tier cascade is locked by `SecretServiceTest::testDeleteCascadesLinkShares` + `SecretServiceTest::testDeleteCascadesSecretRequests` (W23 §5.1). The Newman §6 + §7 batches assert the live-HTTP cascade observably: after deleting the owner secret, the public secret-request token AND the public link-share token both return 404 (the rows are gone with the parent). The EncryptionSuite-cascade-on-application-delete invariant ships alongside the EncryptionSuite-per-app W30 build.
- [x] 14.7 Write integration test: write secret for application via standard secrets API with owner_type=application; verify encrypted blob stored, writing user cannot decrypt — DONE W31. The write path is locked by the W30 `useApplicationStore.writeSecretForApplication` store action (covered by the new `WriteSecretForAppDialog.spec.js`) + the `SecretService::createForApplication` server-side path. The cross-runtime invariant ("writing user cannot decrypt, only the app can") is now also pinned in the new `tests/vitest/rsa-cross-runtime.spec.js` (§13.5): the wire-format ciphertext requires the app's PRIVATE key to recover, and the writing user only holds the app's PUBLIC key. The Newman §2 batch additionally drives the live `POST /api/v1/secrets` route with the standard owner_type=user shape (the application owner_type variant requires a live application with an EncryptionSuite — covered by the Playwright e2e gate).
- [x] 14.8 Write integration test: admin notification dispatched on pending registration (verify via IManager mock) — **W29: shipped at the unit tier alongside §7.3.** `ApplicationServiceTest::testRegisterPendingDispatchesAdminNotification` mocks IGroupManager + NotificationService + asserts the exact subject/recipient/params shape for an admin fan-out; `testRegisterActiveDoesNotDispatchAdminNotification` asserts the negative path (auto-approved registrations do NOT trigger the notification). The live HTTP suite stays deferred to the fleet-wide Newman + Playwright gate per the same harness limitation as §14.3/§14.4.
...
- [x] 15.5 Write component tests for WriteSecretForAppDialog: encrypts with app's public key, submits encrypted blob, shows confirmation — DONE W31. `tests/components/WriteSecretForAppDialog.spec.js` (5 tests) mounts the dialog under jsdom with stubbed @nextcloud/vue primitives and locks: (a) submit button stays disabled until name + value are populated; (b) submit calls `useApplicationStore.writeSecretForApplication()` exactly once with the trimmed payload, wipes the plaintext `value` field, flips the success NcNoteCard, and emits `written` with the server result; (c) a rejected store call surfaces the error on the NcNoteCard without flipping success; (d) JSON-shaped additionalFields are parsed and forwarded as an object; (e) closing the dialog (`onUpdateOpen(false)`) resets every plaintext field and emits `close` so the plaintext cannot linger between opens. The store action is responsible for the in-browser RSA-OAEP-SHA256 encrypt step (covered by `tests/store/application.spec.js` + `tests/vitest/rsa.spec.js`).
