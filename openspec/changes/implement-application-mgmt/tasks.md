...
- [~] 9.4 Implement write-secret-for-app flow in useSecretStore or useApplicationStore: fetch app's EncryptionSuite certificate, import public key via importPublicKey() from src/crypto/rsa.js, encrypt secret fields with rsaEncrypt(), POST to /api/v1/secrets with owner_type=application and owner_id — DEFERRED: client-side flow needs (a) an attribute-stable shape for `owner_type=application` on the Secret entity surface (currently the entity carries `owner_type`/`owner_id`, but SecretService still keys everything off `user_id`) and (b) certificate fetch via a public `applications/{id}/certificate` route. Both land with the EncryptionSuite-per-app pass; the crypto primitives (`importPublicKey` / `rsaEncrypt`) already exist in `src/crypto/rsa.js`.
...
- [~] 10.5 Create `src/components/ApplicationSecretsPanel.vue` using CnDataTable: lists secrets attributed to the application (name, type, created_at); click navigates to SecretDetail (read-only for user -- encrypted blobs only); "Write Secret" button opens WriteSecretForAppDialog — DEFERRED with §10.2 ApplicationDetail; the panel is its sidebar widget and only renders meaningfully once owner_type=application secrets are queryable via SecretService (gated on §9.4 server-side `owner_type` filter).
- [~] 10.6 Create `src/components/WriteSecretForAppDialog.vue` using NcDialog, NcInputField: form with name, key (password), login (optional), URL (optional), additional fields; on submit: fetches app's public certificate, encrypts with rsaEncrypt, POSTs secret; shows confirmation on success — DEFERRED with §9.4 + §10.2: needs the per-app certificate fetch endpoint and the `owner_type=application` Secret API extension before its submit handler is buildable end-to-end.
