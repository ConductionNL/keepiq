...
- [~] 3.3 Implement CSR validation in register(): `openssl_csr_get_public_key($csrPem)` to extract public key, `openssl_pkey_get_details($pubKey)['bits']` to check >= 4096, throw InvalidArgumentException on invalid CSR format or insufficient key size — DEFERRED: ApplicationService::register() currently accepts an optional CSR and stores it verbatim for the EncryptionSuite-provisioning flow (which lives in CertificateAuthorityService). Hardening with explicit `openssl_csr_get_public_key` + bits >= 4096 check + targeted exception lands with the EncryptionSuite-per-app provisioning pass — at that point the same validation runs in both register() and the suite signer.
