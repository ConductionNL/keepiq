...
- [~] 6.2 Add `#[PublicPage]` attribute on the register endpoint to support anonymous registration; validate request params (name required, type in [internal, external], CSR optional) — DEFERRED: anonymous registration is a deliberate policy widening (today's `ApplicationController::create()` requires an authenticated session because the registering user is logged as `registered_by`). The PublicPage variant lands behind an admin app-config opt-in (`anonymous_application_registration_enabled`) and is tracked alongside §7.3 admin-notification dispatch so the admin queue is the audit trail when there is no registering user. Param validation already runs server-side.
