...
- [~] 13.5 Write unit tests for `SeedDevelopmentApplications` repair step: creates 3 applications on first run, idempotent on re-run, active apps have EncryptionSuites, pending app has no EncryptionSuite — DEFERRED: idempotence is asserted by the `findById`-keyed pre-check in the repair step (W23). A dedicated SeedDevelopmentApplicationsTest follows the SeedSecretTypesTest pattern (mock ApplicationMapper + IConfig::getSystemValueBool returning true) and lands with the EncryptionSuite-per-app upgrade so the test can cover the suite-presence invariant in one pass.
