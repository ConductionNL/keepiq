...
- [~] 15.5 Write component tests for WriteSecretForAppDialog: encrypts with app's public key, submits encrypted blob, shows confirmation — DEFERRED with §10.6: the dialog doesn't exist yet (its server-side write-secret-for-app contract is still outstanding). Test pattern (NcDialog stub + axios spy asserting no plaintext in the POST body) is documented in `tests/dialogs/SecretShareDialog.spec.js` and reused once the dialog lands.
