...
- [~] 7.3 Implement admin notification dispatch in ApplicationService::register(): when status=pending, query admin group via IGroupManager, dispatch `app_pending` notification to each admin user — DEFERRED: NotificationService already accepts subject `app_pending`; the dispatch call inside ApplicationService::register() pairs with §6.2 PublicPage so the audit trail is consistent whether registration is authenticated or anonymous. Lands in the same coordinated cycle.
...
- [~] 14.8 Write integration test: admin notification dispatched on pending registration (verify via IManager mock) — DEFERRED with §7.3: dispatch wiring is not yet in ApplicationService::register(); covering the not-yet-implemented behaviour would be a vacuous assertion. Test lands alongside the dispatch implementation.
