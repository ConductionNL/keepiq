...
- [~] 14.3 Write integration tests for JWT auth: exchange valid assertion (200, access token returned), exchange with invalid signature (401), exchange for pending app (401), use access token to list secrets (200), use expired access token (401) — DEFERRED: doriath's PHP suite is unit-only (mocked mappers, no live HTTP harness). JWT verify / sign paths are covered by JwtAuthServiceTest + JwtAuthMiddlewareTest. The live HTTP suite belongs in the fleet-wide Newman + Playwright gate.
- [~] 14.4 Write integration test: non-admin cannot approve/reject applications (403) — DEFERRED: same harness limitation. The admin guard is asserted at the service layer (ApplicationServiceTest covers `isAdmin === false` => exception).
- [~] 14.5 Write integration test: non-admin cannot delete applications (403) — DEFERRED: same harness limitation. ApplicationServiceTest covers the admin-only delete path.
- [~] 14.6 Write integration test: cascade deletion removes all secrets and EncryptionSuite — DEFERRED: SecretService cascade is now wired (W23 §5.1). EncryptionSuite cascade-on-application-delete is part of the EncryptionSuite-per-app pass and is not yet implemented; covering it in a live test now would assert a behaviour the code doesn't yet provide.
- [~] 14.7 Write integration test: write secret for application via standard secrets API with owner_type=application; verify encrypted blob stored, writing user cannot decrypt — DEFERRED with §9.4: `owner_type=application` is not yet a first-class write path in SecretService, so a live assertion would be vacuous. Lands in the same pass.
