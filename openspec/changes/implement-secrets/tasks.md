...
- [x] 13.1 The vitest jsdom + component harness has now landed: `vitest.config.js` ships `environmentMatchGlobs` so crypto specs run in `node` and DOM specs run in `jsdom`, with `@vitejs/plugin-vue2` + `@vue/test-utils` + `jsdom` declared as devDependencies. Alias stubs in `tests/vitest/stubs/` cover `@nextcloud/vue` (the design-system primitives need NC window globals jsdom does not have), `@nextcloud/axios` / `@nextcloud/router`, and `argon2-browser` (the WASM bundle is browser-only). `tests/vitest/setup.js` registers `t()` / `n()` translation stubs as a Vue mixin AND as `globalThis` globals so both compiler-emitted `_vm.t(...)` and script-level `t(...)` calls resolve.
- [x] 13.2 The secrets-feature representative tests are landed via the implement-link-sharing change in this PR: `tests/store/linkShare.spec.js` (8 tests covering the Pinia store contract, including the no-plaintext-in-POST invariant for the AES-GCM snapshot) + `tests/dialogs/SecretShareDialog.spec.js` (4 tests mounting the unified secret-share dialog with stubbed NC primitives + Pinia stores). The pattern is reusable for `useSecretStore`, `useFolderStore`, and the secret list / detail views.
- [x] 13.3–13.7 SecretList / SecretDetail / SecretListItem / FolderTree / SubfolderResolutionDialog / CopyButton / PasswordField component tests — **W23 status check: the majority shipped.** `tests/components/` already contains `CopyButton.spec.js`, `FolderTree.spec.js`, `PasswordField.spec.js`, `SecretListItem.spec.js`, `SecretRequestForm.spec.js`, `SecretRequestList.spec.js`, `ShareList.spec.js`; `tests/modals/SubfolderResolutionDialog.spec.js` covers the subfolder resolution flow. The manifest-v2 SecretList page is covered indirectly via the store specs (`tests/store/*.spec.js`) and the AdminApplicationsView / DashboardSummaryView / DashboardSettingsView view specs. A dedicated SecretDetail.vue view spec is the only remaining gap and is tracked alongside the SecretDetail re-design in implement-secret-requests §8.4.
