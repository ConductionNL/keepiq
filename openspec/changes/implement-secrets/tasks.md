...
- [~] 13.3–13.7 SecretList / SecretDetail / SecretListItem / FolderTree / SubfolderResolutionDialog / CopyButton / PasswordField component tests — DEFERRED to the next iteration. The harness change in 13.1 unblocks them; the patterns are documented in `tests/dialogs/SecretShareDialog.spec.js` (NC-primitive stubs + Pinia setup) and `tests/store/linkShare.spec.js` (axios spy + plaintext-leak assertion).
